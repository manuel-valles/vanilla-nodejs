...
const alertUserToStatusChange = (newCheckData) => {
  ...
  sendTwilioSms(newCheckData.phone, msg, (err) => {
    if (err)
      return debug('\x1b[31m%s\x1b[0m', 'Could not send sms alert to user who had a state change in their check.', err);
...
const validateCheckData = (originalCheckData) => {
  ...
  originalCheckData.timeoutSeconds =
    isValidTimeoutSeconds(originalCheckData.timeoutSeconds) && originalCheckData.timeoutSeconds;
  ...
  if (requiredKeys.some((key) => !originalCheckData[key]))
    return debug('\x1b[31m%s\x1b[0m', 'One of the checks is not properly formatted. Skipping it!');
...
const allChecks = () =>
  _data.list('checks', (err, checks) => {
    if (err || !checks || checks.length === 0)
      return debug('\x1b[31m%s\x1b[0m', 'Could not find any checks to process');
